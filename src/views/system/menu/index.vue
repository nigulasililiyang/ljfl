  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="600px" :append-to-body='true'>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-row>
          <el-col :span="24">
            <el-form-item  v-if="form.menuType != 'G' "  label="上级菜单">
              <treeselect
          ...
          <el-col :span="24">
            <el-form-item v-if="form.menuType != 'F' && form.menuType != 'G' " label="菜单图标">
              <el-popover
          ...
          <el-col :span="24">
              <el-form-item v-if=" form.menuType == 'G' "  label="所属分组">
              <el-select v-model="form.menuGroup" placeholder="菜单分组" clearable size="small">
                <el-option
                  v-for="dict in menuGroups"
                  :key="dict.dictValue"
                  :label="dict.dictLabel"
                  :value="handleMenuGroup(dict.dictValue)"
                />
              </el-select>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="12">
            <el-form-item v-if="form.menuType != 'F' && form.menuType != 'G' " label="是否外链">
          ...
          <el-col :span="12">
            <el-form-item v-if="form.menuType != 'F' && form.menuType != 'G' " label="路由地址" prop="path">
          ...
          <el-col :span="12">
            <el-form-item v-if="form.menuType != 'M' && form.menuType != 'G' " label="权限标识">
              <el-input v-model="form.perms" placeholder="请权限标识" maxlength="50" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item v-if="form.menuType != 'F' && form.menuType != 'G' " label="菜单状态">
...
export default {
  ...
  created() {
    ...
    this.getDicts("sys_menu_group").then(response => {
      this.menuGroups = response.data;
    });
  },
  methods: {
    handleMenuGroup(menuGroupId){
      return Number.parseInt(menuGroupId);
    },
    // 选择图标
    selected(name) {
      this.form.icon = name;
    ...
    // 菜单分组字典翻译
    menuGroupFormat(row, column) {
      if (row.menuType == "F") {
        return "";
      }
      return this.selectDictLabel(this.menuGroups, row.menuGroup);
    },
    // 取消按钮
    cancel() {
      this.open = false;
