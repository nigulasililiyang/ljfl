<template>
  <div class="app-container">
    ...
    <el-table stripe class="workflow" v-loading="loading" :data="ossobjectList" @selection-change="handleSelectionChange" :row-class-name="getRowClassName">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" min-width="40px">
...
<script>
import { listOssobject, getOssobject, delOssobject, addOssobject, updateOssobject, exportOssobject,listOssobjectByDevice,downloadFile  } from "@/api/v1/system/ossobject";
...
export default {
  ...
  data() {
    return {
      ...
      fileAccept : '.doc,.docx,.xls,.xlsx,.pdf,.png,.jpg,.jpeg,.gif,.bmp,.zip',
      // 表单参数
      form: {},
      // 表单校验
      rules: {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.queryParams.objectTableName = 'device';
      this.queryParams.objectId = this.objectId;

      listOssobjectByDevice(this.queryParams).then(response => {
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          this.form.objectTableName = 'device';
