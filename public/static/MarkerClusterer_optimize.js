 ...
 * 基于Baidu Map API 1.2。
 * @link    MarkerClusterer.js渲染聚合点性能优化,参考博文:http://www.cnblogs.com/lightnull/p/6184867.html;
 * @extends 
...
(function(){
    ...
    MarkerClusterer.prototype._createClusters = function(){
        ...
        var searchMarkers = this._searchMarkers || [];
        var searchMarkerLen = searchMarkers.length

        var len = this._markers.length;
        for (var i = 0; i < len; i++) {
            if(this._clusters[i]){
              var flag = false;
              for(var j=0; j < searchMarkerLen; j ++) {
                // 判断聚合点是否包含目标Marker
                flag = this._clusters[i].isMarkerInClusterBounds(searchMarkers[j]);
                if(flag) {
                  break;
                }
              }
              this._clusters[i].render(flag);  
            }
        }
    };
    ...
    /**
     * 设置聚合点需要判断高亮的目标Marker数组。
     * @param {Array<Marker>} searchMarkers 判断聚合点高亮的目标Marker数组。
     */
    MarkerClusterer.prototype.setSearchMarkers = function(searchMarkers) {
      return (this._searchMarkers = searchMarkers);
    };
     ...
     * @return 无返回值
     */
    Cluster.prototype.render = function(flag){
        var len = this._markers.length;
     ...
     * @return 无返回值。
     */
    Cluster.prototype.updateClusterMarker = function (flag) {
        ...
        this._clusterMarker.setText(this._markers.length, flag);
